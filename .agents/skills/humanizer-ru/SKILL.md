...
name: humanizer-ru
license: MIT
description: Редактор русского текста - убирает признаки AI-генерации, канцелярит и воду, сохраняя факты и добавляя голос. Поддерживает SEO-режим: сохраняет длину, структуру, FAQ, таблицы, CTA и ключевые фразы статьи. 37 паттернов AI-слопа с жёсткими запретами, детерминированным линтером и детектом артефактов копипаста. Use when пользователь просит "отредактируй", "убери воду", "сделай живым", "humanize", "очисти от AI", "перепиши человечнее", говорит что текст "звучит как нейросеть", готовит AI-черновик к публикации, или спрашивает "это писала нейросеть?", "проверь на AI", "палится ли текст". Для SEO-статьи используй SEO-режим. Do NOT use for написания текста с нуля, перевода, проверки орфографии и пунктуации, нерусского текста, кода и конфигов, юридических и академических текстов.
---
...
Включай для статьи, гайда, FAQ-материала или другого текста под поисковый запрос. До аудита прочитай [references/seo-mode.md](references/seo-mode.md) полностью.

В SEO-режиме не сокращай итоговый текст более чем на 5 процентов без прямого разрешения. Сохраняй H1-H3, порядок и число разделов, таблицы, списки, FAQ, CTA, ссылки и ключевые фразы. Правь только обнаруженные паттерны, ритм и неясные предложения. Не вырезай полезное объяснение только потому, что оно написано спокойно или подробно.
...
7. **Запусти линтер по чистовику** (без changelog и цитат «до»):
   ...
   - В SEO-режиме после линтера запусти `python3 scripts/seo_guard.py исходник.md чистовик.md --keyword "основной ключ"`. Исправь каждый `ERROR` до сдачи.
