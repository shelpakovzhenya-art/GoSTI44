  ...
  return <>
    ...
    <header className={`header${scrolled ? " is-scrolled" : ""}${open ? " menu-open" : ""}`} onKeyDown={event => { if (event.key === "Escape" && open) { setOpen(false); menuButton.current?.focus(); } }}>
      <div className="container header-inner">
        ...
        <ContactLinks/>
        <a className="button button-yellow header-book" href="#booking"><CmsText id="navigation.004" /><ArrowUpRight size={16}/></a>
      ...
      {open && <nav id="mobile-navigation" className="mobile-nav" aria-label="Мобильная навигация">{links.map(([title,id]) => <a key={id} href={/^(https?:|mailto:|tel:|\/|#)/.test(id)?id:`#${id}`} onClick={() => setOpen(false)}>{title}<ArrowUpRight size={18}/></a>)}<a href={site.phoneHref}><Phone size={18}/>{site.phone}</a><a className="button button-yellow" href="#booking" onClick={() => setOpen(false)}><CmsText id="navigation.005" /><ArrowUpRight size={18}/></a><ContactLinks labels/></nav>}
    </header>
    <div className="mobile-bottom"><a href={site.phoneHref} aria-label="Позвонить Светлане"><Phone size={20}/></a><a className="button button-yellow" href="#booking"><CmsText id="navigation.006" /><ArrowUpRight size={18}/></a></div>
  </>;
}
