...
export function Booking() {
  ...
  return <div className="booking-box">
    ...
    {open && <div id="booking-widget" className="booking-widget"><iframe src={site.booking} title="Выбор дат и бронирование гостевых домов «Танжерин»" allow="payment"/><p><CmsText id="embeds.004" /><a href={site.booking} target="_blank" rel="noreferrer"><CmsText id="embeds.005" /><ArrowUpRight size={13}/></a>.</p></div>}
...
export function ReviewsWidget() {
  const {site}=useSite();
  const [open,setOpen]=useState(false);
  return <div className="reviews-widget-wrap"><button className="button button-outline" onClick={()=>setOpen(!open)} aria-expanded={open} aria-controls="yandex-reviews">{open?"Свернуть отзывы":"Читать отзывы Яндекс Карт на сайте"}{open?<X size={17}/>:<ArrowUpRight size={17}/>}</button>{open&&<div id="yandex-reviews" className="reviews-widget"><iframe src={site.reviewWidget} title="Официальный виджет отзывов Яндекс Карт о гостевых домах «Танжерин»"/><a className="text-link" href={site.yandex} target="_blank" rel="noreferrer"><CmsText id="embeds.008" /><ArrowUpRight size={15}/></a></div>}</div>;
}
