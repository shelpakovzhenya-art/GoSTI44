...
function Lightbox({start,onClose}:{start:number;onClose:()=>void}) {
  ...
  return <dialog ref={dialog} className="lightbox" aria-label="Фотогалерея гостевых домов" onCancel={event=>{event.preventDefault();onClose();}} onKeyDown={event=>{if(event.key==="ArrowRight")step(1);if(event.key==="ArrowLeft")step(-1);}}><button className="icon-button lightbox-close" onClick={onClose} aria-label="Закрыть фотогалерею"><X/></button><figure><div className="lightbox-image"><Image src={photos[index].src} alt={photos[index].alt} fill sizes="90vw"/></div><figcaption aria-live="polite"><span>{photos[index].alt}</span><span>{index+1} / {photos.length}</span></figcaption></figure><div className="lightbox-buttons"><button className="icon-button" onClick={()=>step(-1)} aria-label="Предыдущее фото"><ChevronLeft/></button><button className="icon-button" onClick={()=>step(1)} aria-label="Следующее фото"><ChevronRight/></button></div></dialog>;
}
