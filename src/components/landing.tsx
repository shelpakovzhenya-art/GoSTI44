...
export default function Landing() {
  ...
  return <div id="top"><ScrollReveal/><Navigation/><main id="main">
    ...
    <section className="section rules-band" id="rules"><div className="container rules-section"><div><div className="eyebrow"><CmsText id="landing.058" /></div><h2><CmsText id="landing.059" /></h2><p><CmsText id="landing.060" /><br/><CmsText id="landing.061" /></p><a className="text-link" href={site.phoneHref}>{site.phone} <ArrowUpRight size={18}/></a></div><div className="faq">{rules.map((rule,index)=><details key={rule.title}><summary><span className="faq-number">0{index+1}</span>{rule.title}<ChevronDown size={18}/></summary><p>{rule.text}</p></details>)}</div></div></section>
