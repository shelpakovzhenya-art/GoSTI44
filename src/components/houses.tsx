...
import { ArrowUpRight, Bath, BedDouble, Check, ChevronLeft, ChevronRight, Maximize2, Leaf, Users, X } from "lucide-react";
...
export function Houses() {
  ...
  return <><div className="houses-grid">{houses.map((house,index)=><HouseCard key={house.id} house={house} index={index} onSelect={()=>setSelected(house)}/>)}</div>{selected && <HouseDialog key={selected.id} house={selected} onClose={()=>setSelected(null)}/>}</>;
}
