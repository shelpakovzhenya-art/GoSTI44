...
function HouseCard({house, onSelect}: { house: House; onSelect: () => void }) {
  ...
  return <article className={`house-card house-${house.tone}`}>
    ...
    <div className="house-copy"><div className="house-specs"><span><Maximize2 size={15}/>{house.areaApproximate ? "≈ " : ""}{house.area} <CmsText id="houses.009" /></span><span><Users size={15}/><EditableText id={`field:${house.id}:guests`} value={house.guests}/> <CmsText id="houses.010" /></span><span><BedDouble size={15}/>{house.bedrooms} <CmsText id="houses.011" /></span></div><h4 className="house-promise"><EditableText id={`field:${house.id}:title`} value={house.title}/></h4><p><EditableText id={`field:${house.id}:description`} value={house.description}/></p><div className="house-tags"><span><CmsText id="houses.012" /></span><span><CmsText id="houses.013" /></span><span><EditableText id={`field:${house.id}:feature`} value={house.feature}/></span></div><div className="house-actions"><button onClick={onSelect}><CmsText id="houses.014" />{" "}<EditableText id={`field:${house.id}:name`} value={house.name} uppercase/> <ArrowUpRight size={19}/></button><a href="#booking"><CmsText id="houses.015" /><ArrowUpRight size={17}/></a></div></div>
  </article>;
}
